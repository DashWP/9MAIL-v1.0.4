...
class Email_Builder {
	...
	private function __construct() {
		...
		add_action( 'save_post_emtmpl', array( $this, 'save_post' ) );
		add_action( 'save_post_emtmpl_block', array( $this, 'save_post_block' ) );
		add_filter( 'manage_emtmpl_posts_columns', array( $this, 'add_column_header' ) );
		add_action( 'manage_emtmpl_posts_custom_column', array( $this, 'add_column_content' ), 10, 2 );
		...
		add_action( 'wp_ajax_emtmpl_search_post', array( $this, 'search_post' ) );
...
//	    Send test result
	...
	public function remove_meta_boxes() {
		if ( get_current_screen()->id == 'emtmpl' ) {
	...
	public function delete_permalink( $link ) {
		...
		return 'emtmpl' === $post_type ? '' : $link;
	}

	public function register_custom_post_type() {

		$labels = array(
			'name'               => _x( '9MAIL - WordPress Email Templates Designer', 'Post Type General Name', '9mail-wordpress-email-templates-designer' ),
			'singular_name'      => _x( '9MAIL - WordPress Email Templates Designer', 'Post Type Singular Name', '9mail-wordpress-email-templates-designer' ),
			'menu_name'          => esc_html__( '9MAIL', '9mail-wordpress-email-templates-designer' ),
			'parent_item_colon'  => esc_html__( 'Parent Email', '9mail-wordpress-email-templates-designer' ),
			'all_items'          => esc_html__( 'All Emails', '9mail-wordpress-email-templates-designer' ),
			'add_new_item'       => esc_html__( 'Add New Email Template', '9mail-wordpress-email-templates-designer' ),
			'add_new'            => esc_html__( 'Add New', '9mail-wordpress-email-templates-designer' ),
			'edit_item'          => esc_html__( 'Edit Email Templates', '9mail-wordpress-email-templates-designer' ),
			'update_item'        => esc_html__( 'Update Email Templates', '9mail-wordpress-email-templates-designer' ),
			'search_items'       => esc_html__( 'Search Email Templates', '9mail-wordpress-email-templates-designer' ),
			'not_found'          => esc_html__( 'Not Found', '9mail-wordpress-email-templates-designer' ),
			'not_found_in_trash' => esc_html__( 'Not found in Trash', '9mail-wordpress-email-templates-designer' ),
		);

		$args = array(
			'label'               => esc_html__( '9MAIL - WordPress Email Templates Designer', '9mail-wordpress-email-templates-designer' ),
			'labels'              => $labels,
			'supports'            => [ 'title' ],
			'hierarchical'        => false,
			'public'              => false,
		...
		register_post_type( 'emtmpl', apply_filters( 'emtmpl_register_post_type', $args ) );

		$labels = array(
			'name'               => _x( 'Blocks', 'Post Type General Name', '9mail-wordpress-email-templates-designer' ),
			'singular_name'      => _x( 'Block', 'Post Type Singular Name', '9mail-wordpress-email-templates-designer' ),
			'menu_name'          => esc_html__( 'Blocks', '9mail-wordpress-email-templates-designer' ),
			'parent_item_colon'  => esc_html__( 'Parent Email', '9mail-wordpress-email-templates-designer' ),
			'all_items'          => esc_html__( 'Blocks', '9mail-wordpress-email-templates-designer' ),
			'add_new_item'       => esc_html__( 'Add New Block', '9mail-wordpress-email-templates-designer' ),
			'add_new'            => esc_html__( 'Add New', '9mail-wordpress-email-templates-designer' ),
			'edit_item'          => esc_html__( 'Edit Block', '9mail-wordpress-email-templates-designer' ),
			'update_item'        => esc_html__( 'Update', '9mail-wordpress-email-templates-designer' ),
			'search_items'       => esc_html__( 'Search', '9mail-wordpress-email-templates-designer' ),
			'not_found'          => esc_html__( 'Not Found', '9mail-wordpress-email-templates-designer' ),
			'not_found_in_trash' => esc_html__( 'Not found in Trash', '9mail-wordpress-email-templates-designer' ),
		);

		$args = array(
			'label'               => esc_html__( 'Blocks', '9mail-wordpress-email-templates-designer' ),
			'labels'              => $labels,
			'supports'            => array( 'title' ),
			'hierarchical'        => false,
			'public'              => false,
			'show_ui'             => true,
			'show_in_menu'        => 'edit.php?post_type=emtmpl',
		...
		register_post_type( 'emtmpl_block', apply_filters( 'emtmpl_block_register_post_type', $args ) );
	}

	public function builder_page( $post ) {
		if ( ! in_array( $post->post_type, [ 'emtmpl', 'emtmpl_block' ] ) ) {
			return;
		}
        ...
        <div id="emtmpl-right-sidebar">
			...
			$boxes = [
				'settings'   => [
					'title' => esc_html__( 'Settings', '9mail-wordpress-email-templates-designer' ),
					'func'  => [ $this, 'email_type_box' ]
				],
				'testing'    => [
					'title' => esc_html__( 'Testing', '9mail-wordpress-email-templates-designer' ),
					'func'  => [ $this, 'email_testing_box' ]
				],
				'admin_note' => [
					'title' => esc_html__( "Admin's note for this template", '9mail-wordpress-email-templates-designer' ),
					'func'  => [ $this, 'admin_note' ]
				],
				'exim_data'  => [
					'title' => esc_html__( "Data", '9mail-wordpress-email-templates-designer' ),
					'func'  => [ $this, 'exim_data' ]
				],
			];
	...
	public function email_type_box( $post ) {
		$languages_selected = get_post_meta( $post->ID, 'emtmpl_setting_languages', true );
		extract( [
			...
			'common_emails'      => Utils::get_common_emails(),
			'admin_emails'       => Utils::get_admin_emails(),
			'user_emails'        => Utils::get_user_emails(),
			'languages_selected' => is_array( $languages_selected ) ? $languages_selected : [],
		] ); // @codingStandardsIgnoreLine
	...
	protected function sanitize_email_structure( $raw_data ) {
		if ( empty( $raw_data ) ) {
			return $raw_data;
		}

		$data = json_decode( wp_unslash( $raw_data ), true );
		$data = $this->data_clean( $data );

		if ( ! empty( $data['rows'] ) ) {
			foreach ( $data['rows'] as $row_i => $row ) {
				if ( empty( $row['cols'] ) || ! is_array( $row['cols'] ) ) {
					continue;
				}

				foreach ( $row['cols'] as $col_i => $col ) {
					if ( empty( $col['elements'] ) || ! is_array( $col['elements'] ) ) {
						continue;
					}

					foreach ( $col['elements'] as $el_i => $element ) {
						if ( empty( $element['type'] ) || $element['type'] !== 'html/text' ) {
							continue;
						}
						...
						if ( ! empty( $element['content']['text'] ) ) {
							...
							$data['rows'][ $row_i ]['cols'][ $col_i ]['elements'][ $el_i ]['content']['text'] = $content;
						}
					}
				}
			}
	...
	public function save_post( $post_id ) {
		if ( ! current_user_can( 'edit_posts' ) || ! isset( $_POST['post_status'] ) || ! in_array( $_POST['post_status'], [ 'publish', 'draft' ] ) ) {
			return;
		}
		...
		foreach ( $keys as $key ) {
			if ( isset( $_POST[ $key ] ) ) {
				...
				if ( $key === 'emtmpl_email_structure' ) {
					$value = $this->sanitize_email_structure( $_POST[ $key ] );
				}
	...
	public function save_post_block( $post_id ) {
		...
		$keys = [ 'emtmpl_email_structure', 'emtmpl_custom_css', 'emtmpl_admin_note' ];
	...
	public function get_lorem_ipsum_string() {
		return sprintf( "<p>%s</p><br/><p>%s</p><br/><p>%s</p><a href='#'>%s</a>",
			esc_html__( "Email content is displayed here. You can modify the attributes in paragraph section.", '9mail-wordpress-email-templates-designer' ),
			esc_html__( "If ignore this template for an email, let's add this shortcode {{ignore_9mail}} to that email content.", '9mail-wordpress-email-templates-designer' ),
			esc_html__( "You can modify some attributes for the links in email in the Link section:", '9mail-wordpress-email-templates-designer' ),
			site_url() );
	}
	...
	public function send_test_email() {
		if ( isset( $_POST['nonce'], $_POST['email'], $_POST['data'] ) && wp_verify_nonce( sanitize_key( $_POST['nonce'] ), 'emtmpl_nonce' ) ) {
			$data  = sanitize_text_field( wp_unslash( $_POST['data'] ) );
			$data  = json_decode( $data, true );
			$files = ! empty( $_POST['attachments'] ) ? wp_unslash( $_POST['attachments'] ) : '';
			...
			$subject    = EMTMPL_PR_CONST['plugin_name'] . ' ' . esc_html__( 'test email template', '9mail-wordpress-email-templates-designer' );
			...
			$error_mess = esc_html__( "Mailing Error Found:", '9mail-wordpress-email-templates-designer' );

			if ( $this->send_test_error_message ) {
				$error_mess .= $this->send_test_error_message;
			}

			$message = $result ? esc_html__( 'Email was sent successfully', '9mail-wordpress-email-templates-designer' ) : $error_mess;
	...
	public function add_column_header( $cols ) {
		$cols = [
			...
			'title' => esc_html__( 'Email subject', '9mail-wordpress-email-templates-designer' ),
			'note'  => esc_html__( 'Note', '9mail-wordpress-email-templates-designer' ),
			'date'  => esc_html__( 'Date', '9mail-wordpress-email-templates-designer' )
		];
	...
	public function post_row_actions( $action, $post ) {
		if ( $post->post_type === 'emtmpl' ) {
			unset( $action['inline hide-if-no-js'] );
			$href   = admin_url( "post.php?action=emtmpl_duplicate&id={$post->ID}" );
			$action = [ 'emtmpl-duplicate' => "<a href='{$href}' onclick='this.style.visibility=\"hidden\";'>" . esc_html__( 'Duplicate', '9mail-wordpress-email-templates-designer' ) . "</a>" ] + $action;
		}
	...
	public function add_filter_dropdown() {
		if ( get_current_screen()->id === 'edit-emtmpl' ) {
			...
			echo '<select name="emtmpl_filter">';
			echo "<option value=''>" . esc_html__( 'Filter by type', '9mail-wordpress-email-templates-designer' ) . "</option>";
	...
	public function change_text_add_title( $title ) {
		if ( get_current_screen()->id == 'emtmpl' ) {
			$title = esc_html__( 'Add Email Subject', '9mail-wordpress-email-templates-designer' );
			echo "<div class='emtmpl-subject-quick-shortcode'><i class='dashicons dashicons-menu'> </i><ul> </ul></div>";
		}
