jQuery(document).ready(function ($) {
...

    {
        ...
        viWecQuickSC.on('click', '.dashicons-menu', function () {
            viWecQuickSC.find('ul').html(viWecShortcodeListValue.map(sc => `<li>${sc}</li>`));
    ...
    const getEmailStructure = () => {
        let dataArray = {}, container = $('#emtmpl-email-editor-wrapper'),
            editorContainer = $('#emtmpl-email-editor-content');
        ...
        $(viWecEditorArea).find('.emtmpl-block').each(function (rowIndex, rowBlock) {
            let row = $(rowBlock).find('.emtmpl-layout-row');
            let templateBlock = $(rowBlock).find('.emtmpl-template-block');
    ...
    function viWecSendTestEmail() {
        ...
        $('.emtmpl-attachment-el input').each(function () {
            ...
            if (typeof rows[rowIndex] === 'string' || typeof rows[rowIndex] === 'number') {
                ...
                if (selectedOption) {
                    let renderData = selectedOption.data,
                        row = $(`<div class="emtmpl-block"><div class="emtmpl-template-block" data-type="blocks" data-block="${templateID}"></div></div>`),
                        _rows = ViWec.renderRows(renderData.rows, false);

                    row.find('.emtmpl-template-block').append(_rows);
                    row.handleRow();
                    // row.find('.emtmpl-edit-outer-row-btn').remove();
                ...
                let row = $(viWecTmpl('emtmpl-block', {type: rows[rowIndex].props.type, colsQty: rows[rowIndex].props.dataCols}));

                if (!addEvent) row.find('.emtmpl-column-control').remove();

                row.find('.emtmpl-layout-row').css(rows[rowIndex].props.style_outer);
                ...
                row.find('.emtmpl-column').each(function (colIndex) {
                    ...
                    col.find('.emtmpl-column-sortable').css(colStyle);
                ...
                if (addEvent) {
                    ...
                    row.removeClass('emtmpl-block');
                    row.find('.emtmpl-element').removeClass('emtmpl-element');
                    row.find('.emtmpl-column-sortable').removeClass('emtmpl-column-sortable');
                    row.find('.emtmpl-layout-row').removeClass('emtmpl-layout-row');
                }
    ...
    ViWec.viWecDrawTemplate = (viWecTemplate) => {
        ...
        $('#emtmpl-email-editor-wrapper').css(bg);
        $('#emtmpl-email-editor-content').css({width: viWecTemplate['style_container']['width']}).attr('data-responsive', viWecTemplate['style_container']['responsive'] || 380);
    ...
    const runApp = {
        ...
        emailTypeChange() {
            const filterShortcode = (emailType) => {
                let shortcode = Object.keys(viWecParams.commonShortcode) || [];

                if (viWecParams.shortcode[emailType]) {
                    let emailTypeSc = Object.keys(viWecParams.shortcode[emailType]);
                    viWecShortcodeListValue = [...new Set([...shortcode, ...emailTypeSc])];
                } else {
                    viWecShortcodeListValue = [...new Set([...shortcode])];
                }
            };
            ...
            $('.emtmpl-set-email-type').on('change', function () {
                ...
                filterShortcode(emailType);

                if (emailType === 'default') {
                    $(`.emtmpl-element[data-type="html/recover_content"] .emtmpl-element-handle-inner`).hide();
                } else {
                    $(`.emtmpl-element[data-type="html/recover_content"]`).remove();
                    $('.emtmpl-layout-handle-outer').show();
                }
        ...
        hideElements(type) {
            ...
            if (type === 'default') {
                $(`#emtmpl-components-list .emtmpl-control-btn[data-type='html/recover_content']`).parent().removeClass('emtmpl-hidden');
            } else {
                $(`#emtmpl-components-list .emtmpl-control-btn[data-type='html/recover_content']`).parent().addClass('emtmpl-hidden');
            }
        },

        attachmentFile() {
            $('.emtmpl-attachment-el').on('click', '.emtmpl-remove-attachment', function (e) {
                $(this).parent().remove();
            });

            let images = wp.media({multiple: true});
            $('.emtmpl-add-attachment-file').on('click', function () {
                let list = $('.emtmpl-attachments-list');
                images.on('select', function (e) {
                    ...
                    selection.each(function (attachment) {
                        ...
                        if (list.find(`[value=${id}]`).length === 0) {
                            let el = $(`<div class="emtmpl-attachment-el vi-ui button tiny">
                                        <a href="${viWecParams.uploadUrl}?item=${id}" target="_blank">${filename}</a>
                                        <input type="hidden" name="emtmpl_attachments[]" value="${id}">
                                        <i class="emtmpl-remove-attachment dashicons dashicons-no-alt"></i>
                                    </div>`);

                            el.on('click', '.emtmpl-remove-attachment', function () {
                                el.remove();
                            });
